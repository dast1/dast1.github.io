...
One current instance of that last pattern is what TypeSafe describes as a System One model, under the name Jev. As they describe it, the caller supplies state and typed questions, and the model returns a choice, a score, or a yes-or-no probability rather than prose. The name collides with Kahneman's System 1. I am not using it that way, and I am not adopting the product. I have not measured its latency, its calibration, or its accuracy. I am interested in the shape: a component that is allowed to be uncertain, inside an action set the caller fixed. Classifiers have had that shape for a long time. The decision plane is the idea. The implementation should be whichever one is accurate enough, cheap enough, and inspectable enough for that particular decision.[^3]
...
The tenth time, the exploration is waste. The path is known. Continuing to ask a general model to reinvent it — re-read the tools, re-derive the steps, restate the policy in a new paragraph — spends latency and money to produce variance you did not order. An established workflow should be called, not rediscovered.
...
I am not describing a product, a network, or a company. I am describing a separation I want in systems I am willing to trust with a repeated action. The next question is what happens when generation, the decision, and the execution are no longer on the same machine. That is a governance problem before it is a topology problem, and it is a different essay.
...
[^2]: Hypothesis. How large the constrained share is depends on the product, and I have not counted it. The architectural test — if the outputs can be enumerated, do not ask a generator to invent them — is the part I would keep even if the share is smaller than I think.

[^3]: The decision-plane abstraction is the claim. Classifiers, regressors, small decision models, constrained decoding, decoder-logit classification, and a typed decision model are a map of patterns, not a comparison I have run. Jev, described by TypeSafe as a System One model, is one public instance of the typed shape: state in, a choice or a score or a yes-or-no probability out, no prose. I have not independently verified that description, and this essay does not use their latency, price, or accuracy figures. Those need a measurement on a stated task before they belong here. Decoder-logit classification — reading a next-token distribution over a fixed label set — needs a citation before publication if the essay ever says anything about its accuracy relative to a trained classifier. It does not say that here.

[^4]: Principle: policy stays deterministic even when classification is probabilistic. The example confidence is illustrative. The calibration warning is the part that needs evidence. I would not state, as a finding, how often agent thresholds are fit on the wrong base rate. The operational rule stands without that finding: do not gate a side effect on a probability you have not checked against the traffic you actually see.
